...
/* 初始化CANFD1/2的底层和中断 */
static void can12_borad_init(bool use_canfd1,bool use_canfd2)
{
	if(!use_canfd1 && !use_canfd2)
	{
		while(1)
		{
		}
...
{
	/* 初始化的时候这两个参数必定要有一个是true否则卡死在这个borad函数里面 */
	can12_borad_init(use_canfd1,use_canfd2);
...
//    hfdcan1.Init.DataTimeSeg2=;
    ...
    hfdcan1.Instance=FDCAN1;
	  hfdcan1.Init.Mode=FDCAN_MODE_NORMAL;//工作模式，一般就关注正常模式和内外环回测试就行
...
#if   USE_CAN_FD_FOMAT
    /* 设置仲裁阶段MBPS为500KBPS，公式为1/((1/25MHZ)*(1+seg1+seg2))BPS */
    hfdcan2.Init.NominalPrescaler=8;//仲裁阶段时钟8分频25MHZ，1tq=1/(25MHZ)s
    hfdcan2.Init.NominalSyncJumpWidth=10;
    hfdcan2.Init.NominalTimeSeg1=37;
    hfdcan2.Init.NominalTimeSeg2=12;
    
     /* 设置数据阶段MBPS为1MBPS */
    hfdcan2.Init.DataPrescaler=8;//数据阶段时钟8分频25MHZ，1tq=1/(25MHZ)s
...
//    hfdcan2.Init.DataTimeSeg2=;
#endif

#if   USE_CAN_FD_FOMAT
    hfdcan2.Init.FrameFormat=FDCAN_FRAME_FD_BRS;//工作在CANFD模式并且有波特率变速，允许收发最大数据长度64字节的报文
		
    hfdcan2.Init.RxBufferSize=FDCAN_DATA_BYTES_8;//接收buffer可以接收数据长度为8的数据帧
    hfdcan2.Init.RxBuffersNbr=0;//不给他分配RAM，即我们不使用接收buffer

    hfdcan2.Init.RxFifo0ElmtSize=FDCAN_DATA_BYTES_64;//接收FIFO0可以接收数据长度为64的数据帧
    hfdcan2.Init.RxFifo0ElmtsNbr=10;//接收FIFO0可以缓存10条数据长度为64的数据帧

    hfdcan2.Init.RxFifo1ElmtSize=FDCAN_DATA_BYTES_64;//接收FIFO1可以接收数据长度为64的数据帧
    hfdcan2.Init.RxFifo1ElmtsNbr=10;//接收FIFO1可以缓存10条数据长度为64的数据帧
		
	  hfdcan2.Init.TxElmtSize=FDCAN_DATA_BYTES_64;//允许发送64个字节长度的数据帧
#elif USE_STD_CAN_FOMAT
    hfdcan2.Init.FrameFormat=FDCAN_FRAME_CLASSIC;//工作在CAN2.0标准模式，只允许收发最大8字节数据长度的报文
		
	  hfdcan2.Init.RxBufferSize=FDCAN_DATA_BYTES_8;
    hfdcan2.Init.RxBuffersNbr=0;

    hfdcan2.Init.RxFifo0ElmtSize=FDCAN_DATA_BYTES_8;
    hfdcan2.Init.RxFifo0ElmtsNbr=30;

    hfdcan2.Init.RxFifo1ElmtSize=FDCAN_DATA_BYTES_8;
    hfdcan2.Init.RxFifo1ElmtsNbr=30;
    ...
    hfdcan2.Init.StdFiltersNbr=4;//标准ID的过滤器有4条
    hfdcan2.Init.ExtFiltersNbr=3;//扩展ID的过滤器有3条

    hfdcan2.Init.TxBuffersNbr=0;//发送缓冲区个数为0，不需要使用
    hfdcan2.Init.TxEventsNbr=0;//事件发送缓存个数为0不需要用
    hfdcan2.Init.TxFifoQueueElmtsNbr=30;//30个发送FIFO或者发送队列的长度，选队列还是FIFO由下面控制
    hfdcan2.Init.TxFifoQueueMode=FDCAN_TX_FIFO_OPERATION;//选择FIFO模式
	  HAL_FDCAN_Init(&hfdcan2);

    /* 初始化4条标准ID筛选器和3条扩展ID筛选器 */
    can12_fd_init_filter(&hfdcan2);
...
#if   USE_CAN_FD_FOMAT
		FCAN1TxHeader_ST.BitRateSwitch=FDCAN_BRS_ON;//允许波特率变速
		FCAN1TxHeader_ST.ErrorStateIndicator=FDCAN_ESI_ACTIVE;//当前处于主动错误状态
		FCAN1TxHeader_ST.FDFormat=FDCAN_FD_CAN;//帧格式为FDCAN
		FCAN1TxHeader_ST.IdType=FDCAN_STANDARD_ID;
		FCAN1TxHeader_ST.TxEventFifoControl=FDCAN_NO_TX_EVENTS;//没有发送事件FIFO
		FCAN1TxHeader_ST.TxFrameType=FDCAN_DATA_FRAME;//数据帧，CANFD没有遥控帧
		
		FCAN1TxHeader_EXT.BitRateSwitch=FDCAN_BRS_ON;
		FCAN1TxHeader_EXT.ErrorStateIndicator=FDCAN_ESI_ACTIVE;
		FCAN1TxHeader_EXT.FDFormat=FDCAN_FD_CAN;
		FCAN1TxHeader_EXT.IdType=FDCAN_EXTENDED_ID;
		FCAN1TxHeader_EXT.TxEventFifoControl=FDCAN_NO_TX_EVENTS;
		FCAN1TxHeader_EXT.TxFrameType=FDCAN_DATA_FRAME;
		
		FCAN2TxHeader_ST.BitRateSwitch=FDCAN_BRS_ON;//允许波特率变速
		FCAN2TxHeader_ST.ErrorStateIndicator=FDCAN_ESI_ACTIVE;//当前处于主动错误状态
		FCAN2TxHeader_ST.FDFormat=FDCAN_FD_CAN;//帧格式为FDCAN
		FCAN2TxHeader_ST.IdType=FDCAN_STANDARD_ID;
		FCAN2TxHeader_ST.TxEventFifoControl=FDCAN_NO_TX_EVENTS;//没有发送事件FIFO
		FCAN2TxHeader_ST.TxFrameType=FDCAN_DATA_FRAME;//数据帧，CANFD没有遥控帧
		
		FCAN2TxHeader_EXT.BitRateSwitch=FDCAN_BRS_ON;
		FCAN2TxHeader_EXT.ErrorStateIndicator=FDCAN_ESI_ACTIVE;
		FCAN2TxHeader_EXT.FDFormat=FDCAN_FD_CAN;
		FCAN2TxHeader_EXT.IdType=FDCAN_EXTENDED_ID;
		FCAN2TxHeader_EXT.TxEventFifoControl=FDCAN_NO_TX_EVENTS;
		FCAN2TxHeader_EXT.TxFrameType=FDCAN_DATA_FRAME;
#elif USE_STD_CAN_FOMAT
    FCAN1TxHeader_ST.BitRateSwitch=FDCAN_BRS_OFF;//不允许波特率变速，CAN2.0没有这个机制
		FCAN1TxHeader_ST.ErrorStateIndicator=FDCAN_ESI_ACTIVE;//当前处于主动错误状态,CAN2.0没有这个位，随便给
		FCAN1TxHeader_ST.FDFormat=FDCAN_CLASSIC_CAN;//帧格式为标准CAN
		FCAN1TxHeader_ST.IdType=FDCAN_STANDARD_ID;
		FCAN1TxHeader_ST.TxEventFifoControl=FDCAN_NO_TX_EVENTS;//没有发送事件FIFO
		FCAN1TxHeader_ST.TxFrameType=FDCAN_DATA_FRAME;//数据帧,CAN2.0标准允许有遥控帧，我们可以在函数中指定参数，这里先默认数据帧
		
		FCAN1TxHeader_EXT.BitRateSwitch=FDCAN_BRS_OFF;
		FCAN1TxHeader_EXT.ErrorStateIndicator=FDCAN_ESI_ACTIVE;
		FCAN1TxHeader_EXT.FDFormat=FDCAN_CLASSIC_CAN;
		FCAN1TxHeader_EXT.IdType=FDCAN_EXTENDED_ID;
		FCAN1TxHeader_EXT.TxEventFifoControl=FDCAN_NO_TX_EVENTS;
		FCAN1TxHeader_EXT.TxFrameType=FDCAN_DATA_FRAME;
		
		FCAN2TxHeader_ST.BitRateSwitch=FDCAN_BRS_OFF;//不允许波特率变速，CAN2.0没有这个机制
		FCAN2TxHeader_ST.ErrorStateIndicator=FDCAN_ESI_ACTIVE;//当前处于主动错误状态,CAN2.0没有这个位，随便给
		FCAN2TxHeader_ST.FDFormat=FDCAN_CLASSIC_CAN;//帧格式为标准CAN
		FCAN2TxHeader_ST.IdType=FDCAN_STANDARD_ID;
		FCAN2TxHeader_ST.TxEventFifoControl=FDCAN_NO_TX_EVENTS;//没有发送事件FIFO
		FCAN2TxHeader_ST.TxFrameType=FDCAN_DATA_FRAME;//数据帧,CAN2.0标准允许有遥控帧，我们可以在函数中指定参数，这里先默认数据帧
		
		FCAN2TxHeader_EXT.BitRateSwitch=FDCAN_BRS_OFF;
		FCAN2TxHeader_EXT.ErrorStateIndicator=FDCAN_ESI_ACTIVE;
		FCAN2TxHeader_EXT.FDFormat=FDCAN_CLASSIC_CAN;
		FCAN2TxHeader_EXT.IdType=FDCAN_EXTENDED_ID;
		FCAN2TxHeader_EXT.TxEventFifoControl=FDCAN_NO_TX_EVENTS;
		FCAN2TxHeader_EXT.TxFrameType=FDCAN_DATA_FRAME;
#endif
