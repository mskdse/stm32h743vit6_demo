...
void FATFS_TEST_INIT(uint8_t dev,bool ismount)
{
    ...
    usart1_my_printf("f_read res: %d, len: %u\r\n",res,bytes_read);
    if (res == FR_OK && bytes_read > 0)
    {
        read_buf[bytes_read] = '\0';
        usart1_my_printf("read data: [%s]\r\n",read_buf);
    }
    /* ==================== 8. 关闭文件 ==================== */
    res = f_close(&file);
    usart1_my_printf("f_close res: %d\r\n",res);
    ...
    usart1_my_printf("f_unlink %s res: %d\r\n",filepath,res);
    /* ==================== 10. 取消挂载 ==================== */
    res = f_mount(&fs, volume, 0);
    usart1_my_printf("f_unmount %s res: %d\r\n",volume,res);
#else
    /* ==================== 挂载/取消挂载 ==================== */
    res = f_mount(&fs,volume,ismount ? 1 : 0);
    usart1_my_printf("%s %s res: %d\r\n",ismount ? "f_mount" : "f_unmount",volume,res);
#endif
}
