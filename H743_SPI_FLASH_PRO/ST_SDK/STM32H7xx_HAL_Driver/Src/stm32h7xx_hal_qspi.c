...
HAL_StatusTypeDef HAL_QSPI_Abort(QSPI_HandleTypeDef *hqspi)
{
  ...
  if (((uint32_t)hqspi->State & 0x2U) != 0U)
  {
    ...
    if (__HAL_QSPI_GET_FLAG(hqspi, QSPI_FLAG_BUSY) != RESET)
    {
      ...
      /* Wait until TC flag is set to go back in idle state */
      status = QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_TC, SET, tickstart, HAL_MAX_DELAY);
      ...
      if (status == HAL_OK)
      {
        ...
        status = QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_BUSY, RESET, tickstart, HAL_MAX_DELAY);
      }
