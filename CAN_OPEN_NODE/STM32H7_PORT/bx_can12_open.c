...
static void bx_can12_init_filter(FDCAN_HandleTypeDef* hfdcan12)
{ 
	 /* 配置标准ID筛选器1，工作在掩码过滤模式，为了适配can open协议栈，筛选出全部报文 */
   ...
   /* 配置全局过滤器，拒绝非匹配筛选器的标准帧和扩展帧。不允许遥控帧，CAN OPEN已经舍弃遥控帧 */
...
//    hfdcan1.Init.DataTimeSeg2=;
    ...
    hfdcan1.Init.RxFifo0ElmtsNbr=60;
    ...
    hfdcan1.Init.StdFiltersNbr=1;//标准ID的过滤器有1条
    hfdcan1.Init.ExtFiltersNbr=0;//扩展ID的过滤器有0条
    ...
    /* 初始化1条标准ID筛选器和0条扩展ID筛选器 */
    bx_can12_init_filter(&hfdcan1);
		
		/* 配置接收FIFO1的水位线中断，只要收到一个数据就产生中断 */
...
//    hfdcan2.Init.DataTimeSeg2=;
    ...
    hfdcan2.Init.RxFifo0ElmtsNbr=60;
    ...
    hfdcan2.Init.StdFiltersNbr=1;//标准ID的过滤器有1条
    hfdcan2.Init.ExtFiltersNbr=0;//扩展ID的过滤器有0条
...
{
/* 为了适配CAN OPEN，只能发送标准格式的CAN帧，而且不存在遥控帧
	 并且长度定死8字节以内，由上层进行指定，驱动层不做字节限幅处理 */
...
/* 接收函数，从队列中提取我们的数据，取消临界区机制，临界区由CAN OPEN协议栈给定 */
bool bx_can12_get_msg(FDCAN_GlobalTypeDef *CANIndex,RX_FIFO_TYPE* rmsg)
{
	if(FDCAN1==CANIndex)
	{
		...
		{	
			...
			memcpy(rmsg->pdata,FACAN1_RX_FIFO.fifo[FACAN1_RX_FIFO.read].pdata,8);
	...
	else if(FDCAN2==CANIndex)
	{
		...
		{	
			...
			memcpy(rmsg->pdata,FACAN2_RX_FIFO.fifo[FACAN2_RX_FIFO.read].pdata,8);
...
/* 接收消息回调，我们把它们接收到队列当中缓存起来，需要一次性取完，取消临界区机制，临界区由CAN OPEN协议栈给定 */
